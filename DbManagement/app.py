...
SELECT_ALL_STATISTICS = ("select r.resource_name, p.timestamp, p.price from price_statistics p, resources r where r.resource_id = p.resource_id order by p.timestamp;")
    ...
    with connection:
        with connection.cursor() as cursor:
            try:
                ...
                # ORDER BY IN SQL ASSURES SELECT RETURNS CORRECTLY PREPARED DATA
                output = []
                records = cursor.fetchall()
                curr_time = records[0][1]
                prices = []
                for index, record in enumerate(records):
                    if curr_time == record[1] and index != len(records) - 1:
                        resource_data = {}
                        resource_data['resource'] = record[0]
                        resource_data['price'] = record[2]
                        prices.append(resource_data)
